...
%% Last-Updated: 2020-06-15T17:20:01+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Force, inertia, metric\\ in Newtonian relativity and in general relativity%\\{\large ***}%
}
...
mechanics. My desire for this exploration has two main motivations. The
first is to understand the boundary between kinematics and dynamics in
general relativity, and whether a single boundary common to Newtonian and
general relativity can be drawn. The second is a dissatisfaction with the
division between forced and unforced (geodesic) motion common to both
theories as usually presented. Let me explain these two points in more
detail.
...
Several authors
\citep{vandantzig1934,vandantzig1934b,vandantzig1934c,vandantzig1934d,vandantzig1934e,vandantzig1936,vandantzig1937,vandantzig1954}
seem to find this point of view appealing.
...
possible, even if some seem apparently unrelated with it. Two studies will
be especially important for our exploration. The first is a
reinterpretation of the familiar \enquote{\,$\tsum\bm{F} = m\bm{a}$\,}; the
second is a metric-free reformulation of electromagnetism. These studies
may be unfamiliar to some readers and are summarized in \sects***.
...
\citep{kaluza1921,goenner2004,goenner2012,verbinetal2005}).

But this distinction can possibly be eliminated by taking a point of view
opposite to geometrization: instead of trying to reduce force to geometry,
we try to reduce geometry to force. This point of view is suggested by
Newtonian mechanics; in is the reinterpretation of
\enquote{\,$\tsum\bm{F} = m\bm{a}$\,} mentioned above. In this possible to
reformulate Newtonian dynamics as the requirement that the sum of all
forces on a body be always zero, just like in the science of statics:
$\tsum \bm{F} = 0$. Among these forces, however, we must include the
\emph{inertial} force:
...
Trying to carry this point of view over to general relativity, one wonders
whether the dynamics can be reformulated as the requirement that the sum of
...
-- could seem to represent exactly such a requirement. But there are
difficulties with this interpretation. The main difficulty is that the
definition of the stress-energy-momentum tensor $\yT$ (more precisely the
constitutive equations for this tensor) often requires metric notions; the
...
therein]{samohyletal1987_r2014} %[see also][]{samohyl1975b}
