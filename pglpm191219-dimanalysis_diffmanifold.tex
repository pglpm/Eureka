...
%% Last-Updated: 2020-01-11T14:32:01+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Dimensional analysis on differential manifolds\\ and in general relativity%\\{\large ***}%
...
There seem to be insecurity and wrong ideas among some students and even
some researchers in relativity, regarding the dimensions of tensors and of
tensor components, the effect of tensor operators on their dimension, and
the dimension of constants in field equations. I've met, for example, with
the statement that the components of a tensor should all have the same
dimension; and with calculations of the dimensions of curvature tensors
starting from coordinates with dimensions of length. That statement is
wrong, and that procedure is unnecessary.
...
They say that to find the dimension of some constant in a tensorial
equations it's sufficient to compare the dimensions of the tensors in the
...
texts often use coordinates with dimension of length. Often they even
multiply a timelike coordinate or some tensorial components by powers of
$c$, thus giving the impression that coordinates ought to always be lengths
and that all components of a tensor ought to have the same dimension
\citep[\eg][\sect~37 \eqn~(37.8)]{tolman1934_t1949}[\sect~32
\eqn~(32.15)]{landauetal1939_t1996}[\sect~10.1
\eqn~(10.15)]{adleretal1965_r1975}.

\medskip

In this Note I want to clarify such misconceptions about dimensional
analysis in general relativity and differential geometry, and to illustrate
a simple way of reasoning for solving dimensional-analysis doubts and
problems.

The most important step is the distinction between the \emph{absolute
  dimension} of a tensor and the dimensions of that tensor's \emph{components} in
some coordinate system; this distinction is explained in
\sect~\ref{sec:tensors}. We'll see in particular that \emph{the
  components of a tensor need not have the same dimensions}.

The absolute dimension, introduced by Schouten and Dorgelo
...
the dimensions of the coordinates; it's an intrinsic property of the
...
The relation between absolute and component dimensions, and the dimensional
result of tensor operations such as contraction, derivation, raising or
lowering of indices, and so on, are straightforward to understand and
compute using the usual intuitive rules of dimensional analysis: if two
quantities are summed, then they must have the same dimension; the
dimension of the product is the product of the dimensions; and so on. This
intuitive way of reasoning relies on the coordinate-free, \emph{intrinsic}
view of tensors and other differential-geometrical objects. A brief
reminder of it is given in \sect~\ref{sec:remined}, with further
references.

The results for the main tensor operations and operators are summarized in
...
dimension of the Riemann and Ricci tensors is $\Un$, that is, they are
dimensionless. Some geometric objects or operators may be unfamiliar to
researchers working in general relativity only; their discussion may simply
be skipped by these readers.
...
and the dimension of the constant in the Einstein equations is derived in
\sect~\ref{sec:einstein_eq}, where the two standard results from the
literature are recovered.

The final \sect~\ref{sec:summary} gives a summary of the main points and
results of the present Note.

\medskip

Since the type of a tensor -- that is, the amount and ordering of its
covariant and contravariant \enquote{slots}
\citep[\sect~3.2]{misneretal1970_r1973} -- is important in dimensional
analysis, I often denote it explicitly with the notation
$\yA\i{_{\q}^{\q\q}}$, for example, to indicate that $\yA$ is covariant in
its first slot and contravariant in its second and third slots. Its
components would thus be $(A\i{_{i}^{jk}})$. For brevity I call this a
...
Note that I don't discuss \emph{units} -- here it doesn't matter if the
unit for length is the metre or the centimetre, for example.
...
object that operates on functions defined on the manifold, yielding new
functions, with the properties $\yv(af+bg)=a\yv(f)+b\yv(g)$ and
$\yv(fg)=\yv(f)g+f\yv(g)$ for all functions $f$, $g$ and reals $a$, $b$. A
covector field (1-form) $\yom$ is an object that operates on vector fields,
yielding functions (\enquote{duality}), with the property
...
$\dix\Bigl(\dex[j])=\delt\i{^{i}_{j}}$. These two sets can be used as bases
...
For the full presentation of the intrinsic view I recommend the excellent
texts by Choquet-Bruhat \etal\ \citey{choquetbruhatetal1977_r1996}, Boothby
\citey{boothby1975_r2003}, Abraham \etal\ \citep{abrahametal1983_r1988},
...
\citey[\chap~4]{penroseetal1984_r2003}. \textcolor{white}{If you find this you can claim a postcard from me.}
...
thus have any dimensions: length $\Le$, time $\Ti$, angle $\Un$, energy
...
ensuing sets of covector fields (1-form) $\dix$ and of vector fields
...
$\dim(\dix) = \Xx_{i}$, and the operator (a vector) $\dex$ traditionally
has the inverse dimension: $\dim\dex = {\Xx_{i}}^{-1}$. We'll see later
that these conventions are self-consistent.

For our discussion let's take a concrete example: a contra-co-tensor field
$\yA \equiv \yA\i{^{\q}_{\q}}$. The discussion generalizes to tensors
...
\end{equation}
where $\Aa$ is common to all components. Suppose for example that we're
using coordinates with dimensions
\begin{equation}
...
\emph{components} of $\yA$, but its absolute dimension remains the same.
Formula~\eqref{eq:dim_component} for the dimensions of the components is
consistent under changes of coordinates. For example, in new coordinates
$({x'}^{k})$ with dimensions $({\Xx'}_{k})$, the new components of $\yA$
are
\begin{equation}
...
with the general formula~\eqref{eq:dim_component}. % (You may have noticed
...
expansion~\eqref{eq:expansion_tensor}, it's easy to find out the resultant
absolute dimension of various operations and operators on tensors and
tensor fields.

The tensor product of $\yA\i{^{\q}_{\q}}$ and
$\yB\i{_{\q\q}^{\q}}$, for example, can be written as the sum
\begin{equation}
...
Here is then a summary of the dimensional results of the main
differential-geometric operations and operators, except for the covariant
derivative and related tensors, discussed more in depth in
\sect~\ref{sec:connection} below. In brackets I give the section of
Choquet-Bruhat \etal\ \citey{choquetbruhatetal1977_r1996} where they are
defined.


\begin{itemize}[wide=0pt]
\item \emph{Tensor multiplication} [III.B.5] multiplies dimensions:
  \begin{equation}
  \dim(\yA\otimes\yB) = \dim(\yA)\dim(\yB).\label{eq:tensor_mult}
\end{equation}

\item The \emph{contraction} [III.B.5] of the $i$th and $j$th slots (one covariant
...
\item The \emph{transposition} (swapping) of the $i$th and $j$th slots of a
...
If we consider the manifold as a dimensionless quantity (see \sect*** for
what I mean by this), then the dimension of the tangent or velocity vector
$\dot{C}$ to the curve is
\begin{equation}
...
$\Le^{-2}$, according to the general formula~\eqref{eq:dim_component}, if
and only if the \emph{absolute} dimension of $\yR$ is unity,
$\dim(\yR)=\Un$. So I believe that there's no real contradiction with that
statement and the results~\eqref{eq:dim_torsion_riemann_ricci}. This
possible misunderstanding shows that it's important to distinguish between
absolute dimensions, which don't depend on any specific coordinate choice,
and component dimensions, which do.
...
Let's now consider a metric tensor $\yg\i{_{\q\q}}$. What is its
absolute dimension $\dim(\yg)$? There seem to be two or three choices in
the literature; all three can be derived from an operational meaning of the
metric.


Consider a (timelike) worldline $s \mapsto C(s)$, $s\in \clcl{a,b}$,
between events $C(a)$ and $C(b)$. The metric tells us the \emph{proper
  time} elapsed for an observer having that worldline, according to the
formula
...
Many authors \citep[\eg][\sect~V.62, \eqn~(62.02)]{fock1955_t1964}[\chap~11
\eqn~(11.21)]{curtisetal1985}[\sect~5.3 \eqn~(5.6)]{rindler1969_r1986}[\chap~6 \eqn~(6.24)]{hartle2003} prefer to
include a dimensional factor $1/c$ in front of the
...
\end{equation}
possibly with opposite signature \citep[for an exception, with dimension
$\Ti^{2}$, see][\chap~II p.~25]{kilmister1973}{kilmister1973}. If the
...
the worldline $C$, and then take the difference between its final and
initial times. On the other hand, consider the case when the curve $C$ is
\emph{spacelike}. Its proper length is still defined by the
...
observers (they must be orthogonal to the piece) measure each piece. But
the measurement of each piece actually relies on the measurement of
\emph{proper time}: each observer uses \enquote{radar distance}
\citep[\sect~84]{landauetal1939_t1996}, explained in appendix***. Even if
...
metre \citep[p.~98]{bipm1983}[p.~25]{giacomo1984}.


\medskip

The metric $\yg$ can be considered as an operator mapping vectors to
covectors, which we can compactly write as $\yom=\yg\yv$, rather than
...
not determined by the metric). It has only one non-zero coordinate
...
\end{equation}
and it's easily verified that formula~\eqref{eq:dim_christoffel} still
holds, and also~\eqref{eq:dim_torsion_riemann_ricci} since the expression
of the curvature tensors in terms of the connection coefficients is the
same with or without a metric.

The \emph{scalar curvature} $\ysc$ and the \emph{Einstein tensor}
$\yG\i{_{\q}^{\q}}$,
\begin{equation}
  \label{eq:curv_einst}
  \ysc=\tr\yRi\i{_{\q}^{\q}}\equiv\tr_{23}(\yRi\otimes\yg^{-1}),
  \qquad
...
\begin{equation}
  ...
  \dim(\ysc) \defd \dim(\yG\i{_{\q}^{\q}}) =\dim(\yg)^{-1} \equiv
  \begin{cases}
    \Ti^{-2} \\
    \Le^{-2}
  \end{cases}
...
(3-)stress in Newtonian mechanics. The stress $\yt$ is the projection of
the 4-stress $\yT$ onto a spacelike tangent plane with respect to some
...
assume that such spatial projection preserve the absolute dimension, then
...
therefore a 1-form. Putting these two requirements together we obtain a
...
integration of a form does not change the dimension of the form.
Therefore
\begin{equation}
...
Usually the stress is represented by a co-contravariant tensor
$\yt\i{_{\q}^{\q}}$, however. This is done by contracting the last two slots
of $\yt\i{_{\q\;\q\q}}$ with the inverse of the volume element of the
...
the Newtonian 3-metric to have dimension $\Le^{2}$, it can be shown as done
in \sect~\ref{sec:metric} that its volume element has dimension $\Le^{3}$,
and the inverse volume element, $\Le^{-3}$. Thus we obtain
\begin{equation}
...
Even though in most texts the 4-stress is represented by a tensor of order
2, as above, its most fitting geometrical nature is still shrouded by
mystery from a kinematic and dynamical point of view. There are indications
that it could be more properly represented by a covector-valued 3-form
(equivalent to a tensor $\yT\i{_{\q\;\q\q\q}}$ antisymmetric in the last
three slots), or by a 3-vector-valued 3-form (equivalent to a tensor
$\yT\i{^{\q\q\q}_{\q\q\q}}$ antisymmetric in the first three and last three
slots), for reasons connected with integration, similar to those mentioned
above for the stress $\yt\i{_{\q\;\q\q}}$ See for example the discussion
about \enquote{$^{\displaystyle *}\mathte{T}$} by Misner \etal\
\citey[\chap~15]{misneretal1970_r1973}, the works by Segev
...
constant $\dim(G)=\Ma^{-1}\Le^{3}\Ti^{-2}$ and of the speed of light
$\dim(c)=\Le\Ti^{-1}$ only in this way, with an $8\pi$ factor coming from
the Newtonian limit:
\begin{equation}
  \label{eq:k_Gc}
  \yk =     \begin{cases}
...
appears for example in Fock \citey[\sect~55 \eqns~(55.15)
...
I
first present my favourite choice, which is very close to the points of
view of Synge and Bressan \citep[\sects~III.2--4]{synge1960b}[\sects~15,
18]{bressan1978}, and then the other, probably the most common. I also
discuss a possible misinterpretation of traditional expressions for the
\enquote{length element $\ds^{2}$}, with consequences for the dimension of
the metric tensor.

Quoting Synge \citep[\sect~III.3 pp.~108--109]{synge1960b}: \enquote{We are
  now launched on the task of giving physical meaning to the Riemannian
  geometry \textelp{}. It is indeed a Riemannian \emph{chronometry} rather
  than \emph{geometry}, and the word \emph{geometry}, with its dangerous
  suggestion that we should go about measuring \emph{lengths} with
  \emph{yardsticks}, might well be abandoned altogether in the present
  connection}. 
