...
%% Last-Updated: 2020-01-08T15:58:03+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\eqn~(11.21)]{curtisetal1985}[\chap~6 \eqn~(6.24)]{hartle2003} prefer to
...
Thus we see there that the standard choices for $\dim{\yg}$ are $\Ti^{2}$,
$\Le^{2}$, or possibly $\Un$. My favourite choice is the first,
\eqref{eq:dim_g}, for reasons discussed by Synge and Bressan
\citep[\sects~III.2--4]{synge1960b}[\sects~15, 18]{bressan1978} and vividly
summed up by the former \citep[\sect~III.3 pp.~108--109]{synge1960b}:
\begin{quote}\footnotesize We are now launched on the task of giving physical meaning to the
...
integral~\eqref{eq:proper_time}. Its measurement, however, involves
dividing the curve into very short pieces, and having a specially-chosen
observer measure each piece. The measurement of each piece, moreover,
relies on the measurement of the \emph{time} taken by a lightlike signal to
cover it -- its \enquote{radar distance}***ref***. This procedure is
explained in appendix***. Even if rigid rods are used, their calibration
still relies on a measurement of time -- as clear from the \textsc{iso}
definition of the standard metre \citep[item~3-1.a]{iso2009c}.



\citep[\sect~84]{landauetal1939_t1994}
