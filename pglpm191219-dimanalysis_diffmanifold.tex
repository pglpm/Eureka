...
%% Last-Updated: 2019-12-23T21:10:40+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\item The \emph{contraction} of the $i$th and $j$th slots (one covariant
  ...
  (but without raising or lowering indices; see below):
  \begin{equation}
    \dim(\tr_{ij}\yA) = \dim(\yA).
...
\item The \emph{exterior derivative} of a form has the same dimension of
  ...
  \begin{equation}
    ...
    \label{eq:ext_deriv}
  \end{equation}
This can be proven using Cartan's identity with \eqns~\eqref{eq:lie_der} and~\eqref{eq:inter_prod}.
...
We discuss connections, their covariant derivative, and their curvature
tensors in the next section.

\section{Connection, covariant derivative, curvature tensors}
\label{sec:connection}

\begin{itemize}[wide=0pt]
\item the covariant derivative operator $\nabla$ doesn't alter the
  dimensions either: $\dim(\nabla\yA) = \dim(\yA)$. But note that
