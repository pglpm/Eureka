...
%% Last-Updated: 2019-12-23T17:02:58+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
density $\En = \Le^{-1}\Ma\Te^{-2}$, magnetic flux
...
must have the same dimension. This only happens if the generic component
$A\i{^{i}_{j}}$ has dimension
\begin{equation}
...
sum~\eqref{eq:expansion_tensor}: it's called the \emph{absolute dimension}
...
arguments. I'll drop the adjective \enquote{absolute} when it's clear from
the context.

\begin{itemize}[wide]
\item \emph{Tensor multiplication} $\otimes$ multiplies the dimensions:
  $\dim(\yA\otimes\yB) = \dim(\yA)\dim(\yB)$.

\item Same for the \emph{exterior product} $\land$ of differential forms.

\item Same for the \emph{contraction} of tensors (but without raising or
  lowering indices! see below).

\item \emph{Pull-back} and \emph{push-forward} don't change the dimensions
  of the tensors they map.

\item The \emph{Lie derivative} of a tensor with respect to a vector field $\yv$
  multiplies the dimensions of the tensor and of the vector:
  $\dim(\Li_{\yv}\yA) =\dim(\yv)\dim(\yA)$.

\item  same for the \emph{interior product} $\ii_{\yv}$;

\item the \emph{exterior derivative} $\di$ doesn't alter the dimensions of the
  form on which it operates: $\dim(\di\yom) = \dim(\yom)$ (we could use the
  Cartan identity to check this).

\item  same for the integration of a form over a submanifold;


\item  the covariant derivative operator $\nabla$ doesn't alter the dimensions
  either: $\dim(\nabla\yA) = \dim(\yA)$. But note that
  $\dim(\nabla_{\yv}\yA) = \dim(\yv)\dim(\yA)$.
 
  The dimensional effect of the covariant derivative operator can be
  quickly checked by noting that the expression of $\nabla\yA$ contains the
  following term:
$$
\nabla\yA = \dotsb + \partial_{x^l}A{}^{ij}{}_{k}\;
\partial_{x^i}\otimes\partial_{x^j}\otimes\mathrm{d}x^k\otimes\mathrm{d}x^l
+\dotsb.
$$
From the same expression we also find that

\item the Christoffel symbol $\varGamma{}^i{}_{jk}$ has dimensions
