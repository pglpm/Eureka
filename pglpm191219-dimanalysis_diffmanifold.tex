...
%% Last-Updated: 2020-01-05T19:27:30+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
texts in Lorentzian and general relativity commonly use natural units. They
...
The \emph{torsion} $\yTa\i{^{\bdot}_{\bdot\bdot}}$, \emph{Riemann
  curvature} $\yR\i{^{\bdot}_{\bdot\bdot\bdot}}$, and \emph{Ricci
  curvature} $\yR\i{_{\bdot\bdot}}$ tensors are defined by
...
- \nab_{\clcl{\yu,\yv}}\yw,
\\
\yR\i{_{\bdot\bdot}} \defd \tr_{13} \yR\i{^{\bdot}_{\bdot\bdot\bdot}}.
  \label{eq:riemann_eq}  
\end{gather}
From these definitions, formula~\eqref{eq:dim_cov_der_vect}, and the
dimensional properties of tensor product~\eqref{eq:tensor_mult} and
contraction~\eqref{eq:tensor_contr} we see that \emph{torsion, Riemann, and
  Ricci curvature are dimensionless}:
\begin{equation}
  \label{eq:dim_torsion_riemann_ricci}
  \dim(\yTa\i{^{\bdot}_{\bdot\bdot}}) =
  % \label{eq:dim_riem}
   \dim(\yR\i{^{\bdot}_{\bdot\bdot\bdot}}) =
  % \label{eq:dim_ricci}
   \dim(\yR\i{_{\bdot\bdot}}) =1.
\end{equation}
...
The dimensionless results~\eqref{eq:dim_torsion_riemann_ricci} seem to be
at variance with Misner \etal\ \citey[ pp.~34--35 and
p.~407]{misneretal1970_r1973}, who seem to say that
$\yR\i{^{\bdot}_{\bdot\bdot\bdot}}$ has dimension $\Le^{-2}$. I may be
misunderstanding them; but in any case I stand by the
results~\eqref{eq:dim_torsion_riemann_ricci}.
% misner p 407
\medskip
...
\end{equation}
at all events $C(s)$ in the image of the curve. But from the point of view
of dimensional analysis this definition can only be valid if $\yv$ has
dimension $\Ss^{-1}$. For the general case a constant dimensional factor
needs to be introduced in \eqn~\eqref{eq:integral_curve}.
...
\citep[\sect~V.62, \eqn~(62.02)]{fock1955_t1964} %L^2
 \citep[\chap~11 \eqn~(11.21)]{curtisetal1985} %L^2
