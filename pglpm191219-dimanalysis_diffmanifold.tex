...
%% Last-Updated: 2020-11-03T10:51:01+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\eqn~(10.15)]{adleretal1965_r1975}[\sect~5.7]{penfieldetal1967}[for
exception see \eg][\sect~F.III.280]{truesdelletal1960}[\sect~X]{kitano2013}
tensorial components having dimension of time by powers of $c$. Such common
practices therefore give students the impression that coordinates ought to
always be lengths, and that all components of a tensor ought to have the
same dimension. Yet, students cannot find such rules explicitly stated
anywhere. We'll see shortly that no such rules in fact exist, nor are they
necessary.
...
an object that operates on vector fields, yielding functions, with the
property $\yom(f\yu+g\yv)=f\yom(\yu)+g\yom(\yv)$ for all vector fields
$\yu$, $\yv$ and functions $f$, $g$. The sum of vector or covector fields,
and their products by functions -- let's call this \defquote{linearity} --
are defined in an obvious way. Tensors are constructed from these objects;
but see at the end of this section for a slightly different point of view.
...
notation $\yA\i{_{\q}^{\mul{\q\q}}}$ means that $\yA$ is completely
antisymmetric in its last two contravariant slots.
...
slots of two multiplied tensors by simple juxtaposition. For example, if
...
It's possible to build the tensor-product architecture not on vectors and
covectors, but on multi-vectors and multi-covectors, with their straight
and twisted (also called \defquote{odd} %\citep{derham1955_t1984}
or \defquote{axial}% \citep{truesdelletal1960}
...
\citey[\chap~2]{derham1955_t1984}, Schouten \citey{schouten1924_r1954}; and
Deschamps \citey{deschamps1970,deschamps1981} and Lindell
\citey{lindell2004} for the spaces of multi-vectors and -covectors.

In the notation above, the bars actually denote $k$-vectors and -covectors
for $k>1$. Thus $\yA\i{_{\q}^{\mul{\q\q}}}$ really means that $\yA$ belongs
to the tensor product of 1-covectors and 2-vectors, or equivalently that
it's a 1-covector-valued 2-vector.

To avoid burdening the notation I won't add symbols denoting straight or
twisted orientation, but I'll explicitly say when any particular object has
...
\citep[see][\sect~VII.1 p.~88 for an example of metric tensor with
components having different dimensions.]{fokker1960_t1965}. But this isn't
...
also have equal dimensions. So \emph{if we use a system of coordinates
  ...
  equal dimensions}. This justifies the common practice in the literature.
...
dimension $\Ti$ and spacelike coordinates of dimension $\Le$ (without $c$
factors), then the timelike and spacelike components of the charge-current
...
\citep{deschamps1970}[Appendices]{deschamps1981}{lindell2004}[\sect~F.I.267]{truesdelletal1960}
of a multi-vector $\yv$ and a multi-covector $\yom$.
...
In the following we shall see how both choices affect some dimensional
...
similarly for tensors of other types. Therefore
\begin{equation}
...
4-form, called the volume element
...
actually has a twisted orientation. From the results of
\sect~\ref{sec:tensor_ops} it can be shown that the 4-form above has
...
\end{equation}
independently of whether $\dim(\yg)$ equals $\Li^{2}$ or $\Ti^{2}$.
This convention will be seen to have several advantages. Moreover, the
hypervolume of a four-dimensional region is given by the integral of $\ygv$
and has its same dimension (see \sect~\ref{sec:tensor_ops}); and a region
of \emph{spacetime} should reasonably have dimension $\Li^{3}\Ti$.

In general the metric $\yg$ induces volume, area, and line elements on
three-, two-, and one-dimensional regions. It can be convenient to multiply
these elements by appropriate powers of $c$ so that the region's volume has
intuitive dimensions; for example, $\Li^{3}$ for a spacelike
three-dimensional region and $\Li^{2}\Ti$ for a timelike one. Indeed the
definition of proper time~\eqref{eq:proper_time_c} is doing exactly this,
...
The proper volume element appears in the definition of the \emph{star
  operator} on covectors or forms. The latter is obtained by first rising all
indices of a covector and then taking its inner product with the proper
volume element. For example, for a 2-covector or 2-form $\omega \equiv
\omega\i{_{\q\q}}$,
\begin{equation}
  \label{eq:star_op}
...
dimension depends on the degree of the form it operates on. (I personally
prefer to avoid the star operator and explicitly use the inner product with
the proper volume element \citep[\cf][\sects~4.1--2]{bossavit1991}.)

The \emph{inverse proper volume element} is the 4-vector field (completely
antisymmetric contravariant tensor) $\ygv^{-1}$, with twisted orientation,
having unit inner product \citep[Box 4.1, p. 92, item
4]{misneretal1970_r1973}[see also][]{portamana2019e} with the proper volume
element: $\ygv^{-1} \ii \ygv = 1$. Its intrinsic dimension is therefore
\begin{equation}
...
In a rectangular Cartesian coordinate system $(x^{0}, x^{1}, x^{2}, x^{3})$
with dimensions $(\Ti,\Le,\Le,\Le)$, adapted to an inertial (freely
falling) observer at a specific spacetime event, the components of the
metric tensor $\yg$ and of the proper volume element $\ygv$ at that event
are
\begin{equation}
  \label{eq:inertial_components}
  (g\i{_{ij}}) =
  \begin{pmatrix}
    -c^{2} &0\ \mathrm{ms^{-1}}&0\ \mathrm{ms^{-1}}&0\ \mathrm{ms^{-1}}\\
    0\ \mathrm{ms^{-1}}& 1 & 0 & 0\\
    0\ \mathrm{ms^{-1}}& 0 & 1 & 0\\
    0\ \mathrm{ms^{-1}}& 0 & 0 & 1
  \end{pmatrix} \ ,
  ...
  \gamma\i{_{0\,1\,2\,3}} = 1 \ ;
\end{equation}
the units of the zero components are arbitrary of course, as long as they
have dimension $\Le\Ti^{-1}$. Many authors use a metric with opposite sign.
...
surrounded by some mystery, and will be briefly discussed later. First
let's find the dimension of this tensor by following a heuristic approach,
which also shows the usefulness of intrinsic dimensional analysis on
differential manifolds.

The stress-energy-momentum tensor for a material continuum at a spacetime
event embodies the energy density (including rest energy) $\en$, surface
energy-flux density $\yq_{r}$, momentum density $\yp_{r}$, and stress
(including convected momentum density) $\yt_{sr}$ of the material at that
event; here the indices $r,s \in \set{x,y,z}$. These quantities are
measured by an observer at that event, using a system of one timelike and
three spacelike coordinates $(t,x,y,z)$. If these coordinates have
dimensions $(\Ti,\Le,\Le,\Le)$, then the dimensions of the quantities above
are
\begin{equation}\label{eq:dims_stress_components}
  \begin{aligned}
    ...
    &
    % \label{eq:dim_yq}
    \dim(\yq_{r}) &= \En\Le^{-2}\Ti^{-1} \equiv \Ma\Ti^{-3} \ ,
    \\
    \dim(\yp_{r}) &= \En\Le^{-4}\Ti \equiv \Ma\Le^{-2}\Ti^{-1} \ ,
    &
    % \label{eq:dim_stress}
    \dim(\yt_{rs}) &= \En\Le^{-3} \equiv \Ma\Le^{-1}\Ti^{-2} \ ,
  \end{aligned}
\end{equation}
where the vertical positions of the indices do \emph{not} denote any
variance type.

Suppose we want to construct a spacetime tensor containing these 16
quantities as components. What should the variance type and the intrinsic
dimension of such a tensor be?

Since we have 16 components, this tensor could belong to the tensor product
of two spaces, each spanned by four basis elements. There are four such
spaces: vectors, covectors, 3-vectors\footnote{completely antisymmetric
  contravariant tensors of rank 3.}, and 3-covectors\footnote{completely
  antisymmetric covariant tensors of rank 3.}. Let's use shorthands such as
...
\begin{subequations}\label{eq:bases}
  \begin{align}
    ...
    &&\text{\small }\ (\Ti^{-1}, \Le^{-1}, \dotsc) \ ,% \Le^{-1}, \Le^{-1}) \ ,
    \\
    \label{eq:base_covec}
    &  (\di t, \di x, \di y, \di z) \ \text{:}
    &&\text{\small }\ (\Ti, \Le, \dotsc) \ ,%\Le, \Le) \ ,
    \\
    \label{eq:base_trivec}
    &  (\de_{xyz}, \de_{tzy}, \de_{txz}, \de_{tyx})\ \text{:}
    &&\text{\small }\  (\Le^{-3}, \Le^{-2}\Ti^{-1}, \dotsc)\ ,%\Le^{-2}\Ti^{-1}, \Le^{-2}\Ti^{-1}) \ ,
    \\
    \label{eq:base_tricovec}
    &  (\di xyz, \di tzy, \di txz, \di tyx)\ \text{:}
    &&\text{\small }\  (\Le^{3}, \Le^{2}\Ti, \dotsc) % \Le^{2}\Ti, \Le^{2}\Ti) 
  \end{align}
\end{subequations}
(the orderings are chosen to minimize the minus signs appearing from inner
...
spaces above, in all possible combinations. We thus have 16 possible
alternatives to represent our stress-energy-momentum tensor. Energy density
is intuitively associated with the purely timelike component of this
tensor, stress with the purely spacelike ones, and surface energy-flux
density and momentum density with the mixed timelike-spacelike components.

Consider therefore this first alternative, obtained from the tensor product
of the space~\eqref{eq:base_vec} with itself, and omitting $y$- and
$z$-terms for brevity:
\begin{equation*}
...
\yT\i{^{\mul{\q\q\q}}_{\mul{\q\q\q}}} = \en\  \de_{xyz} \otimes\di xyz 
  ...
  \yp_{x}\  \de_{tzy} \otimes\di yxz 
    ...
    \end{multlined}
\end{align}
Our analysis of the intrinsic dimensions therefore restricts the
stress-energy-momentum tensor to be one of the four alternatives above.

Among these, only the second~\eqref{eq:T_co_co3} is consistent under
rescaling of the spatial coordinates. For example, if we switch from metres
...
dividing the old by $(10\,\textrm{dm}/\textrm{m})^{3}$. Only the second
alternative~\eqref{eq:dim_T_co_co3} is consistent with this:
\begin{equation*}
  \en\  \di t\otimes\di xyz =
  \en'\  \di t\otimes\di x'y'z' \ ,
\end{equation*}
and similarly for all its other components. Finally, the value of the
energy density should not change under a change in the orientation of the
coordinates. This means that the 3-covectors in
alternative~\eqref{eq:T_co_co3} should have a twisted orientation.

\medskip

A heuristic application of intrinsic dimensional analysis, combined with a
simple rescaling and reorienting argument, thus tell us that the
stress-energy-momentum tensor has variance $\yT\i{_{\q\mul{\q\q\q}}}$ --
it's a covector-valued 3-covector, or equivalently a covector-valued
3-form, or equivalently a four-times-covariant tensor completely
antisymmetric in three slots.
% \begin{multline}
%   \label{eq:final_T}
% \yT\i{_{\q\mul{\q\q\q}}} =
% \en\  \di t\otimes(\di x \land \di y \land \di z) -
%   \yq_{x}\  \di t\otimes(\di t \land \di y \land \di z) +{}\\
%   \yp_{x}\  \di x\otimes(\di x \land \di y \land \di z) 
%   -\yt_{x x}\  \di x\otimes(\di t \land \di y \land \di z) + \dotsb
% \end{multline}
The 3-covector or antisymmetric slots have a twisted orientation. It has
the dimension of an \emph{action}:
\begin{equation}
...
without considering intrinsic dimensions. Traditionally we would have
multiplied the components by appropriate powers of $c$ to ensure they all
had the same dimension; but this procedure would have left the variance
...
This form of the stress-energy-momentum tensor indeed appears in Einstein's
original work \citep[\sect~C.9]{einstein1914b}; Cartan
\citep[\sect~13]{cartan1923}; Brillouin
\citep[\cf][\sect~7]{brillouin1924}; Truesdell \amp\ Toupin
...
mechanics; and more recent works \citep[\chap~14 Exercise~14.18,
...
which approach its definition from diverse points of view. (Older texts
often speak of a once covariant and once contravariant
\enquote{V(olume)-tensor} or \enquote{tensor density}, but this is an
object completely equivalent to $\yT\i{_{\q \mul{\q\q\q}}}$, with the same
transformation law; the two are related by a contraction with
$\omega\otimes\omega^{-1}$, where $\omega$ is an arbitrary non-vanishing
4-form. See Schouten \citep[\sect~II.8 p.~30]{schouten1951_r1989}.)

This is also the form obtained with variational principles from a Lagrangean,

[esp.\
Theorem~1]{gotayetal1992}



\mynote{possibly Schouten}
\mynote{add dois}

action: \citep[\sect~21.3]{misneretal1970_r1973}
\citep[\sect~3.3]{hawkingetal1973_r1994}  \citep[\sect~IV.55]{pauli1921_t1958}
...
can first take the inner product of the antisymmetric part by the inverse proper
volume element, obtaining a co-contravariant tensor, and then lower the
new contravariant slot using the metric tensor:
\begin{gather}
  \label{eq:co_co_T}
  \yT\i{_{\q\q}} = \bigl(\yT\i{_{\q\mul{\q\q\q}}} \rii \ygv^{-1} \bigr) \yg
\\\intertext{with intrinsic dimension}
\label{eq:dim_co_co_T}
\begin{aligned}
\dim(\yT\i{_{\q\q}}) &=
\dim\bigl(\yT\i{_{\q \mul{\q\q\q}}}\bigr)\dim(\ygv)^{-1}\, \dim(\yg)
  \\&=
  \begin{cases}
\En\Le^{-1} \equiv \Ma\Le\Ti^{-2} \\
\En\Le^{-3}\Ti^{2} \equiv \Ma\Le^{-1}
  \end{cases}
  \quad\text{if }\dim(\yg)\defd
  \begin{cases}
     \Le^2 \\
...
\end{aligned}
\end{gather}
that is, it has the dimension of a \emph{force}, if the metric has dimension
$\Le^{2}$.
...
\mynote{add note on usefulness of coordinate-free way to settle questions
  as in \sect~\ref{sec:stressenergy}?}
