...
%% Last-Updated: 2020-06-19T15:53:12+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\begin{abstract}\labelsep 0pt%
  \noindent This note provides a short guide to performing dimensional
  analysis in general relativity and in differential geometry. It tries to
  revive Dorgelo and Schouten's notion of \defquote{intrinsic} or
  ...
  expresses the physical or operational meaning of a tensor. The
  ...
  contravariant and trice covariant) and Ricci (fully covariant) curvature
  tensors are dimensionless. The relation between dimension and operational
  meaning for the metric and stress-energy-momentum tensors is 
  discussed.
...
dimensions of the metric and of the curvature tensors, in that case? What
is the dimension of the constant in the Einstein equations?
...
questions like the above. % the dimensions
...
general relativity are in circulation. For example, the notion that the
components of a tensor or the coordinate functions should all have the same
dimension. As we'll see shortly, both notions are false.

Several factors contribute to these misconceptions and
insecurity. Modern texts in Lorentzian and general relativity commonly use
geometrized units. They say that, for finding the dimension of some
constant in a tensorial equation, it's sufficient to compare the dimensions
of the terms in the equation. But this procedure is sometimes not so
immediate to apply, because some tensors don't have universally agreed
dimensions -- prime example the metric tensor. Older texts often use four
coordinates with dimension of length, and base their dimensional analyses
on that specific choice. They even multiply\citep[\eg][p.~72
\eqn~(37.8)]{tolman1934_t1949}[p.~80
...
Dimensional analysis may thus not be as self-evident in general relativity
...
The application of dimensional analysis in general relativity seems to me
most straightforward and self-evident if we rely on the coordinate-free or
...
therefore a natural notion for dimensional analysis in general relativity.
...
\defquote{$A\i{_{\pmb{i}}^{\pmb{j}\pmb{k}}}$}. In our discussion, however,
where the difference between a tensor and its set of components will be
crucial, this abstract-index notation unfortunately lends itself to
...
that transposition a generalization of matrix transposition, and
contraction is a generalization of trace, I'll use the following notation:
...
\item $\yA^{\intercal_{\alpha\beta}}$ is the transposition
  ...
  \begin{equation}
    ...
    ,\dotso)
  \end{equation}
  for each $\bm{\zeta},\bm{\eta}$ of appropriate variance type.
  ...
  \begin{equation}
    ...
    \begin{gathered}
      ...
      \parbox{0.9\columnwidth}{\small for any arbitrary complete and
        linearly independent \;$\set{\bm{u}_{i}}$,
        $\set{\bm{\omega}^{j}}$\\
...
must have the same dimension. The four summands of the right side must also
have the same dimension. Denoting $\Aa \defd \dim(\yA)$, we thus have the four
equations
\begin{equation*}
...
determined by the physical or operational meaning of the tensor. Together
with the dimensions of the coordinates it determines the dimensions of the
components, \eqn~\eqref{eq:four_dims_2d}, which need not be all equal.
