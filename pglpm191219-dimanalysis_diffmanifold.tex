...
%% Last-Updated: 2020-01-10T00:30:40+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
texts in Lorentzian and general relativity commonly use geometrized units. They
say that to find the dimension of some constant in a tensorial equations
it's sufficient to compare the dimensions of the tensors in the equation.
But this is not so immediate, because some tensors don't have universally
agreed dimensions -- prime example the 4-metric tensor. Older texts often
use coordinates with dimension of length. They even multiply a timelike
coordinate or some tensorial components by $c$, thus giving the impression
that coordinates should always be lengths and that the components of a
tensor would all have the same dimension.
...
use notation such as $\yT{}_{\bdot}{}^{\bdot}$ to indicate that the tensor
$\yT$ is covariant in its first slot and contravariant in its second; I
...
\citey{boothby1975_r2003}, Marsden \etal\ \citep{marsdenetal1983_r2007},
Bossavit \citey{bossavit1991}, and more on the general-relativity side
Burke \citey[\chap~2]{burke1980b}, Misner \etal\
...
\item The \emph{exterior derivative} [IV.A.2] of a form has the same
  ...
  \citep[\chap~9 p.~180 Theorem~9.78]{curtisetal1985}[\sect~7.4
  Theorem~7.4.8]{marsdenetal1983_r2007} together with
...
determinant \citep[\sect~7.2]{marsdenetal1983_r2007}, can be obtained by
similar reasoning.
...
\end{equation}
and the dimension of $\yg$ is ambiguous -- probably dimensionless if $\yv$
has dimension of length.

\medskip

The standard choices for $\dim{\yg}$ are thus $\Ti^{2}$, $\Le^{2}$, or
possibly $\Un$. My favourite choice is the first, \eqref{eq:dim_g}, for
reasons discussed by Synge and Bressan
...
\citep[\sect~84]{landauetal1939_t1994}, explained in appendix***. Even if
...
holds, and the other two owing to the same expression of the curvature
tensors from the connection coefficients.
...
\begin{equation}
  ...
  \qquad
  \yG\i{_{\bdot}^{\bdot}} \defd \yRi\i{^{\bdot}_{\bdot}} -
  \tfrac{1}{2} \ysc\;\id\i{^{\bdot}_{\bdot}} 
\end{equation}
have therefore dimension
\begin{equation}
  \label{eq:dim_curv_einst}
  \dim(\ysc) = \dim(\yG\i{_{\bdot}^{\bdot}}) =
  \begin{cases}
    \Ti^{-2} \\
    \Le^{-2} \\
    \Un
