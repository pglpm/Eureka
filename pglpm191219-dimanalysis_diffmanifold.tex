...
%% Last-Updated: 2020-11-09T20:52:18+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
exception see \eg][\sect~F.III.280]{truesdelletal1960}[\sect~X]{kitano2013} \mynote{mcvittie}
...
surrounded by some mystery and will be briefly discussed later. Let's first
try to find the dimension of this tensor with a heuristic approach, which
also shows the usefulness of intrinsic dimensional analysis on differential
manifolds.
...
measured by an inertial observer at that event using a system of one
timelike and three spacelike coordinates $(t,x,y,z)$. If these coordinates
have dimensions $(\Ti,\Le,\Le,\Le)$, then the dimensions of the quantities
above are
...
independent quantities as components (I'm not assuming a priori the
symmetry of this tensor: it can be seen as a consequence of the Einstein
equations, and it only needs to hold for the sum of the
stress-energy-momentum tensors from all kinds of matter). What should the
variance type and the intrinsic dimension of such a tensor be?
...
spaces above; % , in all possible combinations;
...
Note that their kind of orientation, straight or twisted, is also still
undetermined.

To further restrict the possibilities let's consider a couple of
additional, interrelated heuristic arguments. First, the notions of energy
and momentum density, surface energy-flux density, and stress imply some
kind of integration over three-dimensional spacelike or timelike regions.
Such integration needs a 3-form and thus excludes
...
motion). % \citep[\cf][\sect~4.28]{landauetal1939_t1996}
...
flux, and for the momentum density and stress, provided we consider mass as
a spatial topological invariant (for a given observer). Only the second
tensor alternative~\eqref{eq:dim_T_co_co3} above is consistent with these
requirements. Finally, the value of the energy density should not change
under a change in the orientation of the spacelike coordinates. This means
that the 3-covector slot in alternative~\eqref{eq:T_co_co3} should have a
twisted orientation.
% \begin{equation*}
%   \en\  \di t\otimes\di xyz =
...
\citep[\cf][\sect~7]{brillouin1924}, Truesdell \amp\ Toupin's
...
also]{gotayetal1992}, from which it is easily seen that this tensor has the
intrinsic dimension of an action.

Note that some of the works just cited speak of a tensor of variance type
...
has variance type $\yT\i{_{\q}^{\q}_{\mul{\q\q\q\q}}}$. But these two
objects are geometrically equivalent, in the sense that their independent
components have the same transformation law under changes of coordinates
\citep[\sect~II.8 p.~30]{schouten1951_r1989}. They are related by a
contraction with $\omega\otimes\omega^{-1}$, where $\omega$ is an arbitrary
non-vanishing 4-form (this object has only one independent component, which
transforms as the scalar $1$).

\medskip

To obtain a twice covariant tensor to be used in the Einstein equations, we
can first take the inner product of the antisymmetric part by the inverse
proper volume element, obtaining a co-contravariant tensor equivalent
to~\eqref{eq:T_co_contra} above, and then lower the new contravariant slot
using the metric tensor:
\begin{gather}
  \label{eq:co_co_T}
  \yTe\i{_{\q\q}} = \bigl(\yT\i{_{\q\mul{\q\q\q}}} \rii \ygv^{-1} \bigr) \yg
...
\citep[\sect~V.55 p.~198]{fock1955_t1964}

 \citep{mcvittie1956_r1965}
%metric \sect~3.6
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 % Eckart, Synge, Landau, (Gourgoulhon):
 % dim x = L
 % dim g = L^2, -1+1+1+1
 % T^^ = E/L^3, +q/c, +cp, s, dim = E/L^5
 % dim T^^ = E/L^5
 % dim T__ = E/L
 % dim T_,___ = E*T
 ...
 % dim g = L^2, +c^2-1-1-1
 % T^^ = E/c^4/L^3, +q/c^4, +p/c^2, s/c^2 (\sect~V.55 \eqn~(55.02)),
 % T^^ = M/c^2/L^3, +q/c^4, +p/c^2, s/c^2 (\sect~V.55 \eqn~(55.02)),
 ...
 % T^^ = E/c^2/L^3, +q/c^2, +p, s (\sect~4.1 \eqn~(4.104)),
 % T^^ = M/L^3, +q/c^2, +p, s (\sect~4.1 \eqn~(4.104)),
 % dim T^^ = E/L^5 = M/L^3/T^2
 % dim T__ = E*T^4/L^5 = M*T^2/L^3 -> dim c^2T__ = M/L
 % dim T_,___ = M*T
 % Ein = -kc^2 T (\sect~4.1 \eqn~(4.107)),
 % k ~ G/c^2 (\sect~4.2 \eqn~(4.225)),

 % Adleretal:
 % dim x = L,L,L,L (\sect~10.1)
 % dim g = L^2, +c^2-1-1-1 (\sect~10.1 \eqn~(10.9))
 % T^^ = M/L^3, +q/c^2, +p/c, s/c^2 (\sect~10.1 \eqn~(10.15)),
 % dim T^^ = M/L^5
 % dim T__ = M/L
 % dim T_,___ = M*T
