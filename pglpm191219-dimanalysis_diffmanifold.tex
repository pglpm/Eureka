...
%% Last-Updated: 2020-01-06T11:25:16+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
call this a "co-contra-variant tensor".
...
properties. For example, a vector field $\yv \equiv \yv(\dotv)$ is an object that operates on
functions defined on the manifold, yielding new functions, with the
properties $\yv(af+bg)=a\yv(f)+b\yv(g)$ and $\yv(fg)=\yv(f)g+f\yv(g)$ for
all functions $f$, $g$ and reals $a$, $b$. A covector field (1-form) $\yom$
is an object that operates on vector fields, yielding functions
(\enquote{duality}), with the property
...
\citey{boothby1975_r2003}, and on the physics and general-relativity side
Burke \citey[\chap~2]{burke1980b}, Bossavit \citey{bossavit1991}, Misner
\etal\ \citey[\chap~9]{misneretal1970_r1973}, Gourgoulhon
...
with the general formula~\eqref{eq:dim_component}.
...
absolute dimension of various operations on tensors and tensor fields.
...
differential-geometric operators. In brackets I give the section of
...
Regarding operations on differential forms:
...
The \emph{covariant derivative} of the product $f\yv$ of a function and a
vector satisfies \citep[\sect~V.B.1]{choquetbruhatetal1977_r1996}
\begin{equation}
...
the image of the curve. But from the point of view of dimensional analysis
this definition can only be valid if $\yv$ has dimension $\Ss^{-1}$. For
the general case a constant dimensional factor needs to be introduced in
\eqn~\eqref{eq:integral_curve}. I would therefore modify the usual
definition of integral curve to \enquote{$C$ is an integral curve of $\yv$
  if $\yv = k\dot{C}$, where $k$ is a possibly dimensionful constant}.

\section{Metric tensor}
\label{sec:metric}

Let's now consider a metric tensor $\yg\i{_{\bdot\bdot}}$. What is its
absolute dimension $\dim(\yg)$?

The answer depends on the operational meaning of the metric, and there
doesn't seem to be a standard choice***
...
Here I offer my personal point of view. We can use the metric to measure
the "length" of (timelike or spacelike) paths in spacetime. The "length" of
a path $c(s)$ with $s\in [a,b]$ is
$$
\int_a^b\!\!\!\mathrm{d}s\;
