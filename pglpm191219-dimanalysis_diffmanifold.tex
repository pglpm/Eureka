...
%% Last-Updated: 2020-01-11T15:45:17+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
intuitive way of reasoning with which you can quickly and consistently
...
the dimensional result of tensor operations such as contraction,
derivation, raising or lowering of indices. This way of reasoning is
presented and illustrated in
...
they must have the same dimension; the dimension of the product is the
product of the dimensions; and so on.

Another important purpose of this Note is to remind you about the forgotten
distinction between the \emph{absolute dimension} of a tensor and the
dimensions of that tensor's \emph{components} in some coordinate system;
this distinction is explained in \sect~\ref{sec:tensors}. The absolute
dimension, introduced by Schouten and Dorgelo
...
This intuitive way of reasoning mentioned above relies on the
coordinate-free, \emph{intrinsic} view of tensors and other
differential-geometrical objects. A brief reminder of it is given in
\sect~\ref{sec:remined}, with further references.


The rest of this Note is mainly an application of that way of reasoning and
the notion of absolute dimension. We'll reveal and rectify a couple of
misconceptions; for example, \emph{the components of a tensor need not have
  the same dimensions}.
...
dimension $\Un$, additive in the previous two, then the function should be
...
$\yG$!) and of the speed of light $\dim(c)=\Le\Ti^{-1}$ only in this way,
with an $8\pi$ factor coming from the Newtonian limit:
\begin{equation}
  \label{eq:k_Gc}
  \yk =     \begin{cases}
   8\pi G/c^{2} \\
...
We have seen that dimensional analysis can be seamlessly done following its
usual standard rules, by adopting a coordinate-free or -invariant view of
tensors. In this view, each tensor has an absolute dimension that doesn't
depend on the dimensions of the coordinates.
...
inverse dimensions of the metric.
...
statements such as \enquote{the tensor $A\i{_{i}^{jk}}$ has dimension
  $\Xx$}, because it isn't clear whether \enquote{$A\i{_{i}^{jk}}$} is
