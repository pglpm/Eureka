...
%% Last-Updated: 2020-01-10T23:02:21+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\makeatletter
\newcommand*{\bdot}{}% Check if undefined
\DeclareRobustCommand*{\bdot}{%
...
researchers in relativity, regarding the dimensions of tensors, of tensor
components, and of dimensional constants in equations. I've met, for
example, with the statement that the components of a tensor should all have
the same dimension; and with calculations of the dimensions of curvature
tensors starting from coordinates with dimensions of length. That statement
is wrong, and that procedure is unnecessary.
...
universally agreed dimensions -- prime example the 4-metric tensor. Older
texts often use coordinates with dimension of length. They even multiply a
timelike coordinate or some tensorial components by $c$, thus giving the
impression that coordinates should always be lengths and that the
components of a tensor would all have the same dimension.
\citep[\eg][\sect~37 \eqn~(37.8)]{tolman1934_t1949}[\sect~32
\eqn~(32.15)]{landauetal1939_t1996}.

In this note I want to clarify some misconceptions about dimensional
analysis in differential manifolds, and to illustrate a simple way of
reasoning to solve dimensional-analysis doubts and problems. This way of
reasoning relies on the coordinate-free, \emph{intrinsic} view of tensors
and other differential-geometrical objects.

Let's start from more general facts about dimensional analysis on differential manifolds.

For dimensional analysis I use ISO conventions and notation. I sometimes
use notation such as $\yA{}_{\bdot}{}^{\bdot}$ to indicate that the tensor
$\yA$ is covariant in its first slot and contravariant in its second; I
call this a \enquote{co-contra-variant tensor}.


[check \citep{aldersley1977}]
...
\citey[\chap~4]{penroseetal1984_r2003}.
...
density $\En \defd \Le^{-1}\Ma\Ti^{-2}$, magnetic flux
$\Fl \defd \Le^{2}\Ma\Ti^{-2}\Cu^{-1}$, temperature $\Te$, and so on.
...
$\yA \equiv \yA\i{^{\bdot}_{\bdot}}$. The discussion generalizes to tensors
...
\begin{equation}
 ...
 \Aa\times \begin{pmatrix}
   \Un & \Le^{-1}\Te & \Le^{-1}\Te & \Le\Ma^{-1}\Ti^{2}\Te
   \\
   \Le\Te^{-1} & \Un & \Un & \Le^{2}\Ma^{-1}\Ti^{2}
   \\
   \Le\Te^{-1} & \Un & \Un & \Le^{2}\Ma^{-1}\Ti^{2}
   \\
   \Le^{-1}\Ma\Ti^{-2}\Te^{-1} & \Le^{-2}\Ma\Ti^{-2} & \Le^{-2}\Ma\Ti^{-2} & \Un
  \end{pmatrix}.
\end{equation}
...
The tensor product of $\yA\i{^{\bdot}_{\bdot}}$ and
$\yB\i{_{\bdot\bdot}^{\bdot}}$, for example, can be written as the sum
\begin{equation}
...
The \emph{torsion} $\yTo\i{^{\bdot}_{\bdot\bdot}}$, \emph{Riemann
  curvature} $\yR\i{^{\bdot}_{\bdot\bdot\bdot}}$, and \emph{Ricci
  curvature} $\yRi\i{_{\bdot\bdot}}$ tensors are defined by
...
- \nab_{\clcl{\yu,\yv}}\yw,
  \label{eq:riemann_eq}  
\\
\yRi\i{_{\bdot\bdot}} \defd \tr_{13} \yR\i{^{\bdot}_{\bdot\bdot\bdot}}.
  \label{eq:ricci_eq}  
\end{gather}
...
\dim(\yTo\i{^{\bdot}_{\bdot\bdot}})\dim(\yu)\dim(\yv) = \dim(\yu) \dim(\yv),
\\
\dim(\yR\i{^{\bdot}_{\bdot\bdot\bdot}})\dim(\yu)\dim(\yv)\dim(\yw) =
\dim(\yu)\dim(\yv)\dim(\yw),
\label{eq:riemann_eq_dim}
\\
\dim(\yRi\i{_{\bdot\bdot}}) = \dim(\yR\i{^{\bdot}_{\bdot\bdot\bdot}}),
...
\begin{equation}
  ...
  \dim(\yTo\i{^{\bdot}_{\bdot\bdot}}) =
  % \label{eq:dim_riem}
   \dim(\yR\i{^{\bdot}_{\bdot\bdot\bdot}}) =
  % \label{eq:dim_ricci}
   \dim(\yRi\i{_{\bdot\bdot}}) =\Un.
\end{equation}
...
Let's now consider a metric tensor $\yg\i{_{\bdot\bdot}}$. What is its
...
%\begin{equation}
%  \label{eq:dim_dt}
 $\Ti \equiv \dim(\inct) = \sqrt{\dim(\yg\i{_{\bdot\bdot}})}$,
%\end{equation}
(independently of the dimension of $s$) and therefore
\begin{equation}\addtocounter{equation}{1}\tag{\theequation a}
    \label{eq:dim_g}
    \dim(\yg\i{_{\bdot\bdot}}) = \Ti^{2}.
  \end{equation}
%\end{subequations}
...
$\yg\i{^{-1}^{\bdot\bdot}}$ is then defined by the formula
\begin{equation}
  \label{eq:inverse_g}
  \yg^{-1}\yg = \id\i{^{\bdot}_{\bdot}},\qquad
  \yg\yg^{-1} = \id\i{_{\bdot}^{\bdot}},
\end{equation}
and obviously
\begin{equation}
  \label{eq:dim_ig}
...
tensor $\ygv\i{_{\bdot\bdot\bdot\bdot}}$, such that
...
$\yA\i{_{\bdot\bdot}} \equiv
\tr_{13}\yA\i{^{\bdot}_{\bdot}}\otimes\yg\i{_{\bdot\bdot}}$ and similarly
for tensors of other types. Therefore
\begin{equation}
  \label{eq:raising_lowering}
  \dim(\yA\i{_{\dotso}_{\bdot}_{\dotso}}) =
  \dim(\yA\i{_{\dotso}^{\bdot}_{\dotso}})\dim(\yg),
\quad
  \dim(\yA\i{_{\dotso}^{\bdot}_{\dotso}}) =
  \dim(\yA\i{_{\dotso}_{\bdot}_{\dotso}})\dim(\yg)^{-1}.
\end{equation}
...
\begin{equation}
  \label{eq:curv_einst}
  \ysc=\tr\yRi\i{_{\bdot}^{\bdot}}\equiv\tr_{23}(\yRi\otimes\yg^{-1}),
  \qquad
  \yG\i{_{\bdot}^{\bdot}} \defd \yRi\i{_{\bdot}^{\bdot}} -
  \tfrac{1}{2} \ysc\;\id\i{_{\bdot}^{\bdot}} 
\end{equation}
have therefore dimension
\begin{equation}
  \label{eq:dim_curv_einst}
  \dim(\ysc) \defd \dim(\yG\i{_{\bdot}^{\bdot}}) =\dim(\yg)^{-1} \equiv
  \begin{cases}
    \Ti^{-2} \\
    \Le^{-2}
  \end{cases}
...
The most fitting geometrical nature of the stress-energy-momentum, from a
kinematic and dynamical point of view, is still shrouded by mystery. In
most relativity texts it is represented by a tensor of order 2, but there
are indications that it could be more properly represented by a
covector-valued 3-form (equivalent to a tensor
$\yT\i{_{\bdot\;\bdot\bdot\bdot}}$ antisymmetric in the last three slots),
or by a 3-vector-valued 3-form (equivalent to a tensor
$\yT\i{^{\bdot\bdot\bdot}_{\bdot\bdot\bdot}}$ antisymmetric in the first
three and last three slots). See for example the discussion about
\enquote{$^{\displaystyle *}\mathte{T}$} by Misner \etal\
...
What are the absolute dimensions of the co-contra-variant stress-energy-momentum tensor $\yT{}_{\bdot}{}^{\bdot}$? We must look for an operational meaning here too. I'll try to  sketch an informal argument that reflects my point of view. The argument can be made more rigorous but that would take too long to do here.
...
So $\nabla\cdot\yT\equiv T{}_i{}^j{}_{;\,j}\;\mathrm{d}x^i$ has the dimensions of energy density, $[\mathrm{M}\,\mathrm{L}^{-1}\mathrm{T}^{-2}]$. The *co-contra-variant* stress-energy-momentum $\yT{}_{\bdot}{}^{\bdot}$ has therefore the same dimensions. But the *co-co-variant* tensor, obtained by contraction with the metric, $\yT_{\bdot\bdot} \equiv \yT\cdot \yg$, has dimensions of energy density times squared time: $[\mathrm{M}\,\mathrm{L}^{-1}]$, a mass over length.
...
In fact the metric tells us the \emph{proper time} $\inct$
elapsed along any (timelike) worldline $s \mapsto C(s)$, $s\in \clcl{a,b}$,
between events $C(a)$ and $C(b)$, by the formula
% \begin{equation}
%   \label{eq:proper_time}
% \inct =  \int_a^b\!\!\!\di s\;
% \sqrt{\abs*{\yg[\dot{C}(s),\dot{C}(s)]}}.
...
We can use the metric to measure the "length" of (timelike or spacelike)
paths in spacetime. The "length" of a path $c(s)$ with $s\in [a,b]$ is
$$
\int_a^b\!\!\!\mathrm{d}s\;
...
tensor $\yR{}^{\bdot}{}_{\bdot\bdot\bdot}$, the Ricci tensor
$\yR_{\bdot\bdot}$, and the Einstein tensor $\yG_{\bdot\bdot}$ are
adimensional – $[\Un]$ – and the scalar curvature has dimensions
...
$$\yG_{\bdot\bdot} = \kappa \yT_{\bdot\bdot}\;.$$
