...
%% Last-Updated: 2020-10-25T11:22:27+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
I prefer to build the tensor-product architecture not on vectors and
covectors, but on multi-vectors and multi-covectors, with their inner and
outer orientations. This elegant and powerful geometric point of view leads
to deeper physical insight, and is gaining popularity in the literature.
For its presentation I recommend the texts of Bossavit
\citey{bossavit1991}, Burke \citey{burke1985_r1987,burke1980b,burke1995},
de~Rham \citey{derham1955_t1984}, Schouten \citey{schouten1924_r1954}; and
Deschamps \citey{deschamps1970,deschamps1981} and Lindell
\citey{lindell2004} for the spaces of multi-vectors and -covectors.

In the notation above, the bars really denote $k$-vectors and -covectors
for $k>1$. Thus $\yA\i{_{\q}^{\mul{\q\q}}}$ really means that $\yA$ belongs
to the tensor product of 1-covectors and a 2-vectors.
...
of a multi-vector $\yv$ and a form $\yom$.
...
\dim(\yTo\i{^{\q}_{\mul{\q\q}}})\dim(\yu)\dim(\yv) = \dim(\yu) \dim(\yv)\ ,
\\
\dim(\yR\i{^{\q}_{\q\mul{\q\q}}})\dim(\yu)\dim(\yv)\dim(\yw) =
\dim(\yu)\dim(\yv)\dim(\yw)\ ,
\label{eq:riemann_eq_dim}
\\
\dim(\yRi\i{_{\q\q}}) = \dim(\yR\i{^{\q}_{\q\mul{\q\q}}})\ ,
...
\begin{equation}
  ...
  \dim(\yTo\i{^{\q}_{\mul{\q\q}}}) =
  % \label{eq:dim_riem}
   \dim(\yR\i{^{\q}_{\q\mul{\q\q}}}) =
  % \label{eq:dim_ricci}
   \dim(\yRi\i{_{\q\q}}) =\Un\ .
\end{equation}
...
A metric tensor and an inner orientation on a four-dimensional spacetime
together determine a unique outer-oriented 4-form, called the metric volume
element
\citep[\sect~V.A.4]{choquetbruhatetal1977_r1996}[\sect~6.2]{abrahametal1983_r1988}.
It can be seen as a completely antisymmetric tensor of rank 4, with only
one non-zero component, given by the square root of the determinant of
...
\end{equation*}
where the sequence $(x^{0}, x^{1}, x^{2}, x^{3})$ must define a
positive orientation. From the results of \sect~\ref{sec:tensor_ops} it can
be shown that the 4-form above has absolute dimension $\dim(\yg)^{2}$ (in
an $n$-dimensional spacetime it has dimension $\dim(\yg)^{n/2}$).
...
\end{equation}
independently of whether $\dim(\yg)$ is $\Li^{2}$ or $\Ti^{2}$.
...
\section{Four-momentum and stress-energy-momentum tensor}
\label{sec:stressenergy}

The 4-momentum of a small body that doesn't emit or absorb energy is given
by its 4-velocity multiplied by its***
