...
%% Last-Updated: 2020-10-27T20:45:10+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
to the tensor product of 1-covectors and a 2-vectors, or equivalently that
...
identity operator (which is a tensor) on the tangent space. Hence
\begin{equation}
  \label{eq:dim_ig}
  \dim(\yg^{-1}) = \dim(\yg)^{-1} \ .
  % \begin{cases}
...
dimension $\dim(\yg)^{n/2}$).

It's convenient to multiply the 4-form above by a power of $c$, and to
define the \emph{proper volume element} $\ygv\i{_{\mul{\q\q\q\q}}}$ as follows:
\begin{equation}
...
$\yt_{sr}$ of the material where $r,s \in \set{x,y,z}$. These quantities
are measured by some observer at that event, using a system of one timelike
and three spacelike coordinates $(t,x,y,z)$. If their dimensions are
$(\Ti,\Le,\Le,\Le)$, then the dimensions of the quantities above are
...
product of two spaces, each spanned by four basis elements. There are four
such spaces: vectors, covectors, 3-vectors\footnote{completely
  antisymmetric contravariant tensors of rank 3.}, and
3-covectors\footnote{completely antisymmetric covariant tensors of rank 3.}.
Let's use shorthands such as
$\de_{txy} \defd \de_{t} \land \de_{x} \land \de_{y}$ and
$\di txy \defd \di t \land \di x \land \di y$. The four spaces then have
coordinate-induced bases
\begin{align}
  \label{eq:base_vec}
  &  (\de_{t}, \de_{x}, \de_{y}, \de_{z})\ ,
  &&\text{\small dim. }\ (\Ti^{-1}, \Le^{-1}, \dotsc) \ ,% \Le^{-1}, \Le^{-1}) \ ,
\\
  \label{eq:base_covec}
  &  (\di t, \di x, \di y, \di z) \ ,
  &&\text{\small dim. }\ (\Ti, \Le, \dotsc) \ ,%\Le, \Le) \ ,
\\
  \label{eq:base_trivec}
  &  (-\de_{xyz}, \de_{tyz}, -\de_{txz}, \de_{txy})\ ,
  &&\text{\small dim. }\  (\Le^{-3}, \Le^{-2}\Ti^{-1}, \dotsc)\ ,%\Le^{-2}\Ti^{-1}, \Le^{-2}\Ti^{-1}) \ ,
\\
  \label{eq:base_tricovec}
  &  (\di xyz, -\di tyz, \di txz, -\di txy)\ ,
  &&\text{\small dim. }\  (\Le^{3}, \Le^{2}\Ti, \dotsc) % \Le^{2}\Ti, \Le^{2}\Ti) 
\end{align}
(the signs are chosen to respect duality). There are therefore $4 \times 4$
possible tensor-product spaces, for instance the one including the
composite basis element $\de_{t} \otimes \de_{t}$, the one including
$\de_{t} \otimes \di xyz$, and so on. We have 16 possible alternatives to
represent our tensor. For each alternative, Energy density is intuitively
associated with the purely timelike composite basis element, stress with
the purely spacelike ones, and surface energy-flux density and momentum
density with the mixed timelike-spacelike basis elements.

Consider this alternative, omitting $y$- and $z$-terms for brevity:
\begin{equation*}
...
%   \end{aligned}
% \end{equation*}
The first and third summands have incompatible intrinsic dimensions
$\En\Le^{-3}\Ti^{-2}$ and $\En\Le^{-5}$. This alternative is therefore
rejected because dimensionally inconsistent. Similar dimensional analyses
on the other fifteen alternatives show that only four are dimensionally
consistent:
\begin{align}
...
\\\shortintertext{with intrinsic dimension}
  ...
  \dim\bigr(\yT\i{_{\q \mul{\q\q\q}}}\bigl) = \En\Ti \equiv \Ma\Le^{2}\Ti^{-1} \ .
\end{gather}
