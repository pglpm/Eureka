...
%% Last-Updated: 2020-02-19T11:33:52+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\firstdraft}{17 February 2020}
\newcommand*{\firstpublished}{17 February 2020}
\newcommand*{\updated}{\ifarxiv***\else\today\fi}
\newcommand*{\propertitle}{Memos on ideal gases%\\{\large ***}%
}% title uses LARGE; set Large for smaller
\newcommand*{\pdftitle}{\propertitle}
\newcommand*{\headtitle}{Memos on ideal gases}
...
\begin{abstract}\labelsep 0pt%
  \noindent Memos on ideal gases, from a thermomechanical viewpoint.
% \par%\\[\jot]
% \noindent
...
\item $t$: time instant
\item $\yr$: position
\item $n$: mole density (a 3-form)
\item $\yv$: velocity
\item $M$: molar mass
\item $p$: pressure
\item $\yte$: temperature
\item $u$: internal energy density
\item $\yq$: heat flow
\end{itemize}
For an ideal gas, the fields $\bigl(n(t,\yr),\yv(t,\yr),\yte(t,\yr)\bigr)$
...
\begin{equation}
  ...
  \nab\yv^{\perp}\defd \nab\yv^{+}-1/3\nab\cdot\yv\, \id
\end{equation}
and note that from the balance of mass below
\begin{equation}
  \label{eq:div_shear_volume}
  \nab\cdot\yv \equiv \tr(\nab\yv^{+}) = -\frac{\de_{t}n + \yv\cdot\nab n}{n}
\end{equation}

The equations below are derived for example in  Samoh\'yl \amp\ Peka\v{r}
\citey[\sects~3.5--3.7]{samohyletal1987_r2014}; compare also Truesdell \amp\  Muncaster \citey[\chap~I]{truesdelletal1980}.


\section{Balance laws}
\label{sec:balances}

Quantity of matter, force (in an inertial frame), energy:
\begin{gather}
  \label{eq:matter_bal}
  \de_{t}n +\nab\cdot(n\yv) = 0
\\
  \label{eq:force_bal}
  M\de_{t}(n\yv) + M\nab\cdot(n\yv\otimes\yv) - \nab\cdot\yT = 0
\\
  \label{eq:energy_bal}
  \de_{t} u + \nab\cdot(u \yv)  + \nab\cdot\yq - \yT\con\nab\yv^{+} = 0
\end{gather}

Constitutive equations for heat flow, stress, internal energy:
\begin{gather}
  \label{eq:const}
  \yq = -k(n,\yte)\,\nab\yte %- \kappa(n,\yte)\,\nab n
  \\
  \yT = -[R n \yte - \zeta(n,\yte)\,\nab\cdot\yv] \id + \eta(n,\yte)\,\nab\yv^{\perp}
  \\
  u = c\yte%f(n,\yte)+\yte\de_{\yte}f % c \yte 
  \\
  k,\eta,\zeta\ge 0
\end{gather}
$k$ is the thermal conductivity, $\eta$ the shear viscosity, $\zeta$ the
bulk or volume viscosity. Equations for these are given \eg\ in Kannuluik
