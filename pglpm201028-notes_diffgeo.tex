...
%% Last-Updated: 2020-10-28T21:28:56+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
with their possible straight or twisted orientations.


\section{Comparison of exterior-product definitions}
\label{sec:wedge_defs}
...
\textcite{bossavit1991}:
\begin{equation}
  \label{eq:wedge_marsden}
  \begin{split}
    \alpha \land \beta &\defd
    \frac{(\deg \alpha+\deg \beta)!}{(\deg \alpha)!\ (\deg \beta)!}\ 
    A(\alpha \otimes \beta)
  \\
  &= \frac{1}{(\deg \alpha)!\ (\deg \beta)!}
  \sum_{\pi}\sgn(\pi)\ (\alpha \otimes \beta)\circ\pi
\end{split}
\end{equation}
Note that for 1-covectors $\alpha \land \beta = \alpha \otimes \beta -
\beta \otimes \alpha$.

The fact that the definition of multi(co)vector is not unique shows that
the multivector algebra is a separate entity from the tensor algebra, not a
subspace of it \citep[\cf][]{deschamps1970,deschamps1981}.
