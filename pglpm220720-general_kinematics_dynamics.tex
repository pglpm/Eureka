...
%% Last-Updated: 2022-07-21T19:23:16+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
We can represent charge density and current density in one geometrical entity. We use a coordinate system $(t, x, y, z)$; denote the wedge product by juxtaposition: $\di t \land \di x \defs \di t\, \di x$ and so on; abbreviate $\di t\, \di x \defs \di tx$ and so on; and denote twisted differential forms by $\ti$. Then the charge-current density is
\begin{multline}
  \label{eq:charge-current}
  \rho\ \ti xyz +
  j_{x}\ \ti tyz + j_{y}\ \ti tzx + j_{z}\ \ti txy
  \equiv{}\\
  \rho\ \ti xyz + \di t\ (j_{x}\ \ti yz + j_{y}\ \ti zx + j_{z}\ \ti xy)
\end{multline}


% \[ \color{bluepurple}\bm{a} \color{redpurple}\mathbin{\bm{\land}}\color{bluepurple} \bm{b} \]

\section{Twisted objects}
\label{sec:twisted}
...
\begin{gather}
  ...
  \set[\big]{\ti txyz} = +1 \ .
\end{gather}
The minus signs appear when we have $t$ and an even number of other coordinates after the \enquote{$\ti$}.

Note that considering, say, the \emph{function} $t$, we have
\begin{equation}
