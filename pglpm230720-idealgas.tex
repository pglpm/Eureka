...
%% Last-Updated: 2020-07-23T20:01:10+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\yl}{l}
\newcommand*{\yld}{\Dot{l}}
\newcommand*{\yldd}{\Ddot{l}}
\newcommand*{\yno}{n_{\text{r}}}
\newcommand*{\yt}{\theta}
\newcommand*{\yq}{\bm{q}}
\newcommand*{\yv}{\bm{v}}
...
\begin{equation}
  ...
  x(X,t) = \yl(t) X, \qquad X \in \clcl{-1, 1}
\end{equation}
with the $y$ and $z$ coordinates remaining constant. This motion requires
the pressures on the two pistons, at positions $\yl$ and $-\yl$, to be
...
the values of the temperature $\yt[\yl(t), t] \defs \ytb(t)$ and of the
temperature gradient $(\de_{x}\yt)[\yl(t), t] \defs \ygb(t)$ at one piston
...
$\yl(t) > 0$ of one piston, the other having position $-\yl(t)$, so that
the volume is given by $2 \yl(t) \times{}$cross-sectional area.
...
p(x,t) &= R\, n\, \ytb + \tfrac{1}{2} \,R \, n \, \ygb \,
         \frac{x^{2}-\yl^{2}}{\yl} - \yL \,\frac{\yld}{\yl}
         \\&=
...
\\
\label{eq:T_motion}
\yt(x,t) &= \ytb + \tfrac{1}{2}\, \ygb\, \frac{x^{2}-\yl^{2}}{\yl}
\\
\label{eq:u_motion}
  u(x,t) &= \yC \, \ytb +
           \tfrac{1}{2}\, \yC \,\ygb \,\frac{x^{2}-\yl^{2}}{\yl}
\\
\label{eq:q_motion}
q(x,t) &= - K \, \ygb \,\frac{x}{\yl}
\end{align}
where it we have omitted the time-dependence of the quantities $\yl(t)$, $\ypb(t)$,
$\yld(t)$, $\ytb(t)$, $\ygb(t)$
