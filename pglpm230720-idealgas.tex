...
%% Last-Updated: 2020-07-23T16:31:05+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\propertitle}{Affine ideal gas [draft]%\\{\large ***}%
}% title uses LARGE; set Large for smaller
\newcommand*{\pdftitle}{\propertitle}
\newcommand*{\headtitle}{Affine ideal gas}
...
\newcommand*{\yx}{\bm{x}}
\newcommand*{\yX}{\bm{X}}
\newcommand*{\yD}{\mathte{D}}
\newcommand*{\yc}{\bm{c}}
\newcommand*{\ytb}{\Hat{\yt}}
\newcommand*{\ygb}{\widehat{\nabla\yt}}
...
\section{Ideal gas in continuum thermomechanics and in thermostatics}
\label{sec:idealgas_ct_ts}

Let $n$ be the volumic amount of substance, $\yv$ the velocity, $\yp$ the
(tensorial) pressure, $\yt$ the temperature, $u$ the specific internal
energy, $\yq$ the heat flux. They are space-time-dependent fields. Also,
\begin{equation}
  \label{eq:def_shear}
...
In continuum thermomechanics an ideal gas is usually defined by the
...
\begin{gather}
 ...
 R, K, C, \yE, \yZ \ge 0
\end{gather}
$K$ is the thermal conductivity, $\yE$ the shear viscosity, $\yZ$ the
...
These are field equations. The independent fields, defining the state of
the body, are taken to be $(n, \yv,\yt)$. Then pressure, internal energy,
heat flux are local functions of state. The fields satisfy the balance laws
of matter, force, internal energy:
\begin{gather}
  \label{eq:matter_bal}
  \de_{t}n +\nabl\cdot(n\yv) = 0
\\
...
constant with respect to the surface normal. In the following we shall
assume that the body is in a cylindrical container of given cross section,
within movable pistons in the direction $x$, and mass centre at rest at
$\yx = 0$. The pressures on the two pistons must be equal (to keep the mass
centre at rest). We also require that the heat fluxes at the two pistons be
equal in magnitude and opposite.

Under these conditions the symmetric parabolic profile of the temperature
field $\yt$ can be summarized by two numbers, for example the values of the
temperature $\ytb$ and of the temperature gradient $\ygb$ at one piston
surface. These two values can be considered quantities. One more state
quantity is the uniform $n$ or equivalently the volume occupied by the
body, which is equivalent to the specification of the position $\yl >0 $ of
one piston, the other having position $-l$, so that the volume is given by
$2 l \times{}$cross-sectional area.
