...
%% Last-Updated: 2023-11-21T12:53:47+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\yg}{\bm{g}}
\newcommand*{\dg}{\sqrt{g}}
\newcommand*{\ve}{\bm{\sqrt[]{g}}}
\newcommand*{\vi}{\bm{\sqrt[]{g^{-1}}}}
%
...
Take an ordered coordinate system $(t,x,y,z)$, which also defines an orientation. The associated bases for 1-vector and 1-covector fields are $\se{t},\dotsc$ and $\si{t},\dotsc$. The bases for 2-vector and 2-covector fields are $\sse{tx},\dotsc$ and $\ssi{tx},\dotsc$; and so on.

A twisted or outer 3-covector such as $\tw{\sssi{xyz}}$ has an associated outer direction, say positive $t$. This is indicated with a notation such as
\begin{equation}
  \label{eq:convention_twisted_short}
 \ttti{t} \defd \tw{\sssi{xyz}} \ .
\end{equation}

The determinant of the metric $\yg$ is denoted shortly
\begin{equation}
  \label{eq:det_g}
\dg \defd \sqrt{-\det\yg} \ .
\end{equation}
The volume element induced by a metric $\yg$ is denoted (note the boldface)
\begin{equation}
  \label{eq:vol_element}
  \ve \defd \dg\ \tw{\ssssi{txyz}}
\end{equation}
and its corresponding inverse, a twisted 4-vector:
\begin{equation}
  \label{eq:inv_vol_element}
  \vi \defd \frac{1}{\dg}\ \tw{\sssse{txyz}}
\end{equation}

Contraction with the volume element or its inverse establishes a correspondence between outer $n$-vectors and inner $(4-n)$-covectors. In particular
\begin{equation}
  \label{eq:vol_contraction}
  \begin{gathered}
    \vi \cdot \ttti{t} = \frac{1}{\dg}\,\se{t}
    \qquad
\vi\cdot \ttti{x} = \frac{1}{\dg}\,\se{x}
\\
\ve \cdot \se{t} = -\dg\,\ti{t}
    \qquad
\ve \cdot \se{x} = -\dg\,\ti{x}
...
\begin{equation}
  ...
  \sigma^{i}_{j}\, \ttti{i}\otimes\si{x^{j}}
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    \sigma^{i}_{j} &= \text{3-stress}
  \end{aligned}
\end{equation}
measured in the coordinate system $txyz$. (The energy $\epsilon$ and momentum $p_{i}$ are per unit \emph{coordinate} volume $xyz$, not per unit metric volume, or \enquote{per unit mass}, or per unit mole.)
