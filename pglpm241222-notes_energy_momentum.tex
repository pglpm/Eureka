...
%% Last-Updated: 2024-12-25T22:42:02+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
%
\newcommand*{\ti}[1]{\di\varOmega_{#1}}
\newcommand*{\tti}[1]{\di^{2}\varOmega_{#1}}
\newcommand*{\ttti}[1]{\di^{3}\varOmega_{#1}}
...
In the expressions above, $\vi{x^{\mu}}$ are the differentials of the coordinates $x^{mu}$ and form a basis for covector fields, and $\ve{\mu}$ are the dual vectors and form a basis for vector fields. The $\ttti{\mu}$ are volume forms defined as follows:
\begin{equation}
...
\begin{equation}
  ...
  = 0
\end{equation}
where $\Di$ is the covariant exterior derivative. The second is, directly in compoents,
\begin{equation}
  \label{eq:sym_EMt}
...
We can choose a non-zero vector field $\yV$ over a four-dimensional spacetime region, which we shall call \emph{reference field}, and construct an associated 4-current
\begin{equation}
  \label{eq:current_V}
...
\end{equation}
This current is a 3-covector, so it can be integrated over any three-dimensional region of spacetime. What's more, thanks to \eqns~\eqref{eq:div_EMt} and~\eqref{eq:sym_EMt} this current satisfies the following balance law:
\begin{equation}
  \label{eq:balance_V}
  \di\yJJ =
...
The ten equations~\eqref{eq:div_EMt}--\eqref{eq:sym_EMt} satisfied by the energy-momentum tensor can be alternatively formulated as balance laws satisfied by ten currents associated to ten independent reference fields. This is how the balance of energy, of the three components of  momentum, of three components of angular momentum, and of three components of boost-momentum come about.

We speak of energy for the 4-current associated with a timelike reference field. We speak of three components of momentum for the 4-currents associated with three spacelike reference fields. This view of energy and momentum as defined with respect to \enquote{reference vector fields} encompasses other common points of view, especially about energy. An example is the virtual-work point of view: the vector $\yV$ in this case represents a field of virtual displacements. Another clear example is the point of view connected with symmetries and Noether's theorem.


Clearly we have infinite choices of vector fields to associate energy and momentum with. We could also define currents that are somehow hybrids of energy and momentum, or \enquote{momentum} whose components don't really form a vector. How to choose such reference fields?
...
\end{equation}
Normalizing them makes sense in that the displacements represented by these vectors are in terms of unit physical time and unit physical length.
...
\item The energy four-current
  ...
  \end{equation}
  associated with the matter four-velocity $\yUm$ is what we call \emph{internal energy}. Note that this reference field is unrelated to the coordinates. This is why internal energy is an invariant in the Newtonian approximation and even in the exact case.
...
\item The energy four-current
  ...
  \end{equation}
  Note that this difference is a spacelike vector field, therefore kinetic energy has something akin to momentum, so to speak.
...
In Schwarzschild isotropic coordinates typical of the barycentric or geocentric celestial reference systems (BCRS, GCRS) \autocites{kaplan2005,soffeletal2003,petitetal2005,soffeletal2013}, the vector field $\ve{t}$ is a Killing vector field; therefore the associated energy $\yEi + \yEk + \yEp$ is conserved, not just balanced; that is, the right side of \eqn~\eqref{eq:balance_V} is zero.

Thus we see where the arbitrariness in the definition of total energy in Newtonian mechanics comes from: it corresponds to the arbitrariness in choosing a timelike reference field for defining energy.
